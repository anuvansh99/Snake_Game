...
bool MazeGame::movePlayer(char move)
{
    ...
    if (isValidMove(newRow, newCol))
    {
        ...
        player.col = newCol;
        if (move == 'w')
            gameMap[player.row][player.col] = PLAYERUP;
        else if (move == 's')
            gameMap[player.row][player.col] = PLAYERDOWN;
        else if (move == 'a')
            gameMap[player.row][player.col] = PLAYERLEFT;
        else if (move == 'd')
            gameMap[player.row][player.col] = PLAYERRIGHT;
        return true;
    }
...
bool MazeGame::moveSnake()
{
    ...
    if (player.row < newRow && gameMap[newRow - 1][newCol] != OBSTACLE)
        newRow--;
    else if (player.row > newRow && gameMap[newRow + 1][newCol] != OBSTACLE)
        newRow++;
    else if (player.col < newCol && gameMap[newRow][newCol - 1] != OBSTACLE)
        newCol--;
    else if (player.col > newCol && gameMap[newRow][newCol + 1] != OBSTACLE)
        newCol++;
